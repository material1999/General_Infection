    ...
    public static void main(String[] args) throws IOException {
        ...
        */
        ...
        for (int i = 0; i <= 0; i++) {
            ...
            String networkFilePath = Parameters.infectionProbabilityFolder + "inf_sim_" + i + ".csv";
            Network network = Read.ReadCsv(networkFilePath);
            int nodeNumber = network.getNodes().size();
            resultPath = "results/community_sim/community_sim_" + i + ".csv";
            CSVWriter writer = new CSVWriter(new FileWriter(resultPath), ';',
                    CSVWriter.NO_QUOTE_CHARACTER,
                    CSVWriter.DEFAULT_ESCAPE_CHARACTER,
                    CSVWriter.DEFAULT_LINE_END);

            Set comm_nodes = new TreeSet();
            comm_nodes.add(1);
            comm_nodes.add(2);
            comm_nodes.add(3);
            ...
            for (int j = 1; j <= nodeNumber; j++) {
                ...
                Write.WriteCommunities(writer, comm_nodes);
            }
            writer.close();
        }
